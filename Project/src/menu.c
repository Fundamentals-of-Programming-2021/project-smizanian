...
int main_menu(char username[100], SDL_Renderer* sdlMenuRenderer, int letter_counter, bool* music, bool last_game) {
    ...
    char mute_text[] = "mute";
    char unmute_text[] = "unmute";
    char error_text[] = "Last game is not available.";
    ...
    Uint32 item5_box_color = 0xff0000ff;
    ...
    while (shallExit == SDL_FALSE) {
        ...
        // mute/unmute box
        boxColor(sdlMenuRenderer, 0, 0, 50, 30, item5_box_color);
        if ((*(music)) == true) {
            stringColor(sdlMenuRenderer, 5, 10, mute_text, 0xff000000);
        }
        else if ((*(music)) == false) {
            stringColor(sdlMenuRenderer, 2, 10, unmute_text, 0xff000000);
        }
        ...
        // error text
        if (last_game == false) {
            stringColor(sdlMenuRenderer, X_item1-W_items/5, Y_item1+H_items+D_items+H_items + 5 , error_text, 0xff0000ff);
        }
        ...
        // mute/unmute color change
        if (X_of_mouse>0 && X_of_mouse<50 && Y_of_mouse>0 && Y_of_mouse<30) {
            item5_box_color = 0xff0000ff - 0xA1000000;
        }
        else {
            item5_box_color = 0xff0000ff;
        }
                ...
                case SDL_MOUSEBUTTONDOWN:
                    if (sdlEvent.button.button == SDL_BUTTON_LEFT) {
                        ...
                        if (item_selected_in_main_menu != 0) {
                            shallExit = SDL_TRUE;
                            if (item_selected_in_main_menu == 2 && last_game == false) {
                                shallExit = SDL_FALSE;
                            }
                        }

                        // mute/unmute
                        else if (X_of_mouse>0 && X_of_mouse<50 && Y_of_mouse>0 && Y_of_mouse<30) {
                            (*music) = !(*music);
                        }
...
int new_game_menu(SDL_Renderer* sdlNewGameMenuRenderer, int* number_of_players, int* cell_num, int* default_map_number) {
    ...
    char map_number[9] = {0};
    int_to_str(map_number, map_counter);
    /*
...
void user_w_or_l_menu(SDL_Renderer * sdlUserWonRenderer, bool user_won) {
    ...
    switch(CELL_NUM) {
        case 5:
            ...
            break;
        case 6:
            strcpy(image_name1, "usrwn3.bmp");
            break;
        case 7:
            strcpy(image_name1, "usrwn4.bmp");
            break;
        case 8:
            strcpy(image_name1, "usrwn5.bmp");
            break;
        default:
            strcpy(image_name1, "usrwn6.bmp");
    }
